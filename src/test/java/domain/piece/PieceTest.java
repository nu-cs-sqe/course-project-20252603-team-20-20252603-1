...
public class PieceTest {
    ...
    @Test
    public void GetCaptureMoves_WhiteRowHigh_ReturnsNoDiagonals() {
        Pawn pawn = new Pawn(Color.WHITE);
        Position position = EasyMock.createMock(Position.class);
    ...
    @Test
    public void GetCaptureMoves_BlackInterior_ReturnsBothDiagonals() {
        Pawn pawn = new Pawn(Color.BLACK);
        Position position = EasyMock.createMock(Position.class);
        EasyMock.expect(position.getRow()).andStubReturn(5);
        EasyMock.expect(position.getCol()).andStubReturn(4);
        EasyMock.replay(position);
        ...
        assertEquals(2, captures.size());
        assertTrue(captures.containsAll(List.of(new Position(4, 3), new Position(4, 5))));
    }

    @Test
    public void GetCaptureMoves_BlackColLow_ReturnsOneDiagonal() {
        Pawn pawn = new Pawn(Color.BLACK);
        Position position = EasyMock.createMock(Position.class);
        EasyMock.expect(position.getRow()).andStubReturn(5);
        EasyMock.expect(position.getCol()).andStubReturn(1);
        ...
        assertEquals(1, captures.size());
        assertTrue(captures.contains(new Position(4, 2)));
    }

    @Test
    public void GetCaptureMoves_BlackRowLow_ReturnsNoDiagonals() {
        Pawn pawn = new Pawn(Color.BLACK);
        Position position = EasyMock.createMock(Position.class);
