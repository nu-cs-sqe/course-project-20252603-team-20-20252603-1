...
|-----|---------------------|-----------------------------------------------------------------------------|--------------|
| TC1 | color=WHITE         | Pawn created, `getPieceType()`=PAWN, `getColor()`=WHITE, `hasMoved()`=false | :x:          |
| TC2 | color=BLACK         | Pawn created, `getPieceType()`=PAWN, `getColor()`=BLACK, `hasMoved()`=false | :x:          |
...
| ID  | State of the System                                        | Expected output                             | Implemented? |
|-----|------------------------------------------------------------|---------------------------------------------|--------------|
| TC4 | WHITE pawn, `hasMoved`=false, position=`(2,1)` (min col)   | `[(3,1), (4,1)]` - two candidates           | :x:          |
| TC5 | WHITE pawn, `hasMoved`=false, position=`(2,8)` (max col)   | `[(3,8), (4,8)]` - two candidates           | :x:          |
| TC6 | WHITE pawn, `hasMoved`=true, position=`(3,1)`              | `[(4,1)]` - one candidate only              | :x:          |
| TC7 | WHITE pawn, `hasMoved`=true, position=`(7,1)` (row HIGH-1) | `[(8,1)]` - one candidate                   | :x:          |
| TC8 | WHITE pawn, `hasMoved`=true, position=`(8,1)` (row HIGH)   | `[]` - empty list, no forward square exists | :x:          |

**BLACK pawn (moves toward lower rows):**

| ID   | State of the System                                       | Expected output                             | Implemented? |
|------|-----------------------------------------------------------|---------------------------------------------|--------------|
| TC9  | BLACK pawn, `hasMoved`=false, position=`(7,1)` (min col)  | `[(6,1), (5,1)]` - two candidates           | :x:          |
